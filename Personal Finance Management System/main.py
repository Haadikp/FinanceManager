...
def check_alerts(user_id):
    query = f"SELECT alert_type, threshold FROM user_alerts WHERE user_id = {user_id}"
    alerts = execute_query("search", query)
    if not alerts:
        return None
    
    messages = []
    for alert_type, threshold in alerts:
        if alert_type == "expense":
            query = f"SELECT SUM(amount) FROM user_expenses WHERE user_id = {user_id}"
            total_expense = execute_query("search", query)[0][0] or 0
...
def alerts():
    ...
    query = f"SELECT alert_type, threshold FROM user_alerts WHERE user_id = {session['user_id']}"
    alerts = execute_query("search", query)
