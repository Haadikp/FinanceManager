...
def analysis(page):
    ...
    try:
        ...
        df_expense['date'] = df_expense['date'].dt.strftime('%Y-%m-%d')
...
def calculate_tax():
    ...
    if request.method == 'POST':
        ...
        if total_expenses > max_deduction:
            flash(f'Deduction amount cannot exceed ₹{max_deduction} for the {tax_regime} tax regime. Please adjust your deduction.', 'warning')
